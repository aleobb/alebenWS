...
void listarDestinatariosFiltrados(ArrayList* listaDestinatarios, ArrayList* listaNegra, int (*pFuncCompare)(char*,char*), int comparisonType)
{
    ...
    //  cloneListaDestinatarios = (ArrayList*)al_filter(listaDestinatarios, listaNegra, pFuncCompare, 0);
        cloneListaDestinatarios = listaDestinatarios->subListByListCompare(listaDestinatarios, pFuncCompare, comparisonType, listaNegra);
        listaDestinatarios->listDuplicatesRemove(cloneListaDestinatarios, pFuncCompare);
...
/// printf ("Destinatario A: %s \t Destinatario B: %s \t Retorno: %f", pDestinatarioA, pDestinatarioB, retorno );
    return retorno;
}
...
int compararDestinatarioPorMail( Destinatario* pDestinatarioA, Destinatario* pDestinatarioB )
{
    ...
    if ( pDestinatarioA != NULL && pDestinatarioB != NULL )
        retorno = stricmp( pDestinatarioA->email, pDestinatarioB->email );
    //printf ("\n Destinatario A: %s \t Destinatario B: %s \t Retorno: %d", pDestinatarioA->email, pDestinatarioB->email, retorno );
    return retorno;
}
