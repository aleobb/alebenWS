    ...
    else
    {
        ...
        if (pArchivo!=NULL)
        {
            for (i=0; i<sizeArrayPeliculas; i++)
            {
                ...
                //fwrite((char*)arrayPeliculas[i].puntaje,sizeof(int),sizeof(arrayPeliculas[i].puntaje)-1,pArchivo);

                fwrite(DURACION,sizeof(char),strlen(DURACION),pArchivo);
               // fwrite((char*)arrayPeliculas[i].duracion,sizeof(int),sizeof(arrayPeliculas[i].duracion)-1,pArchivo);
